...
def main() -> int:
    ...
    def dump_windows_diagnostics() -> None:
        ...
        script = (
            ...
            "foreach ($p in Get-Process EHT_Analytica) {"
            "  $ws=[math]::Round($p.WorkingSet64/1MB,1);"
            "  Write-Output ('PID=' + $p.Id + ' WS=' + $ws + 'MB ' +"
            "    'Threads=' + $p.Threads.Count + ' Title=' + $p.MainWindowTitle);"
            "  $cim = Get-CimInstance Win32_Process -Filter ('ProcessId=' + $p.Id);"
            "  Write-Output ('CmdLine=' + $cim.CommandLine);"
            "  $mods = @($p.Modules | ForEach-Object { $_.ModuleName });"
            "  Write-Output ('ModuleCount=' + $mods.Count);"
            "  $hit = @($mods | Where-Object {"
            "    $_ -match 'python|torch|c10|cudnn|nvcuda|ucrtbase' });"
            "  Write-Output ('KeyModules=' + ($hit -join ','));"
            "};"
